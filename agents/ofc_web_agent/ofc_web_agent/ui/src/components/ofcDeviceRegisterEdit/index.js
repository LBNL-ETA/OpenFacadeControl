...
const OFCDeviceRegistersConfigEditor = () => {
  ...
  useEffect(() => {
    ...
    const fetchSchema = async () => {
      try {
        ...
        const response = await fetch('/ofc/api/v1/schemas?id=ofc_component_registers_schema.json');
        if (!response.ok) {
    ...
    const fetchOptions = async () => {
      try {
        ...
        const response = await fetch('/ofc/api/v1/register_config_files');        
    ...
    const fetchData = async () => {
      try {
        ...
        console.log(lastSegment);
      	const url = '/ofc/api/v1/register_config_files?id=' + lastSegment;
      	console.log(url);
...
export default OFCDeviceRegistersConfigEditor;
